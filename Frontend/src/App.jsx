...
import { hydratePersistedState } from "./redux/persistedSlice";
...
export default function App() {
      ...
      if (p) {
        ...
        dispatch(
          hydratePersistedState({
            tutorChatHistory: p.tutorChatHistory,
            studyPlan: p.studyPlan,
            studyPlanChapters: p.studyPlanChapters,
            studyPlanExamType: p.studyPlanExamType,
            studyPlanExamDate: p.studyPlanExamDate,
          })
        );
        if (p.progressStats) {
  ...
  const renderContent = () => {
    const isSidebarMode = activeFeature === "dashboard";
    return (
      <div className="w-full h-full relative">
        {/* Sidebar Components */}
        {Object.entries(sidebarComponents).map(([key, comp]) => (
          <div
            key={`sidebar-${key}`}
            className="w-full h-full"
            style={{ display: isSidebarMode && sidebarView === key ? "block" : "none" }}
          >
            {comp}
          </div>
        ))}
        ...
        {/* Feature Components */}
        {Object.entries(featureComponents).map(([key, comp]) => (
          <div
            key={`feature-${key}`}
            className="w-full h-full"
            style={{ display: !isSidebarMode && activeFeature === key ? "block" : "none" }}
          >
            {comp}
          </div>
        ))}
  ...
  const mainBg = theme === "light" ? "bg-[#F8F4EE]" : "bg-[#0E1420]";
  const rootBg = theme === "light" ? "bg-[#F8F4EE]" : "bg-[#0E1420]";
