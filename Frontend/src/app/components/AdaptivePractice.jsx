import { useMemo, useState, useEffect, useRef } from "react";
import { useDispatch, useSelector } from "react-redux";
import { clearPracticeIntent } from "../../redux/practiceIntentSlice";
import {
  Clock,
  CheckCircle,
  XCircle,
  Target,
  TrendingUp,
  Loader2,
  BookOpen,
  RotateCcw,
  Sparkles,
  GraduationCap,
} from "lucide-react";
import { API_BASE } from "../../config/api";
import { saveUserProfile } from "../../api/userApi";
import { store } from "../../redux/store";
import { recordPracticeWeakTopics } from "../../redux/knowledgeGraphSlice";
import { incrementAdaptiveStats } from "../../redux/studySlice";
import { setMockTestScore } from "../../redux/progressSlice";
import { AILoader } from "./AILoader";

const EXAM_OPTIONS = [
  {
    id: "JEE",
    title: "JEE",
    blurb: "JEE Main / Advanced PYQs",
  },
  {
    id: "NEET",
    title: "NEET",
    blurb: "NEET (UG) previous years",
  },
  {
    id: "UPSC",
    title: "UPSC",
    blurb: "CSE Prelims / CSAT style",
  },
];

export function AdaptivePractice() {
  const dispatch = useDispatch();
  const lifetime = useSelector((s) => s.study.adaptiveStats);
  const practiceIntent = useSelector((s) => s.practiceIntent?.intent);
  const [phase, setPhase] = useState("input");
  const [topicInput, setTopicInput] = useState("");
  const [examTrack, setExamTrack] = useState("JEE");
  const [sessionTopic, setSessionTopic] = useState("");
  const [sessionExamTrack, setSessionExamTrack] = useState("JEE");
  const [questions, setQuestions] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedAnswer, setSelectedAnswer] = useState(null);
  const [showFeedback, setShowFeedback] = useState(false);

  const [correctCount, setCorrectCount] = useState(0);
  const [attemptedCount, setAttemptedCount] = useState(0);
  const [wrongItems, setWrongItems] = useState([]);
  const [isMockTestSession, setIsMockTestSession] = useState(false);
  const mockSaveDoneRef = useRef(false);

  const WEAK_TOPIC_INCORRECT_THRESHOLD = 2;
  const wrongCountBySubtopicRef = useRef({}); // { [subKey]: number }
  const weakAddedBySubtopicRef = useRef({}); // { [subKey]: true }

  const question = questions[currentIndex];
  const totalQuestions = questions.length;

  const accuracyPct = useMemo(() => {
    if (attemptedCount < 1) return 0;
    return Math.round((correctCount / attemptedCount) * 100);
  }, [correctCount, attemptedCount]);

  const incorrectCount = attemptedCount - correctCount;

  useEffect(() => {
    const intent = practiceIntent;
    if (!intent?.topicInput?.trim() || !intent?.autoGenerate) return;
    dispatch(clearPracticeIntent());
    setTopicInput(intent.topicInput);
    setExamTrack(intent.examTrack || "JEE");
    handleGenerate({
      topic: intent.topicInput,
      examTrack: intent.examTrack || "JEE",
      mockTest: Boolean(intent.isMockTest),
    });
  }, [practiceIntent]);

  const resetSessionState = () => {
    setQuestions([]);
    setCurrentIndex(0);
    setSelectedAnswer(null);
    setShowFeedback(false);
    setCorrectCount(0);
    setAttemptedCount(0);
    setWrongItems([]);
    setSessionTopic("");
    setSessionExamTrack("JEE");
    setIsMockTestSession(false);
    mockSaveDoneRef.current = false;
    wrongCountBySubtopicRef.current = {};
    weakAddedBySubtopicRef.current = {};
  };

  const handleGenerate = async (overrides) => {
    const t = (overrides?.topic ?? topicInput).trim();
    const track = overrides?.examTrack ?? examTrack;
    const mockTest = Boolean(overrides?.mockTest);
    if (!t) return;

    setLoading(true);
    setError(null);
    mockSaveDoneRef.current = false;
    if (overrides) {
      setTopicInput(t);
      setExamTrack(track);
      setIsMockTestSession(mockTest);
    } else {
      setIsMockTestSession(false);
    }

    try {
      const res = await fetch(`${API_BASE}/api/generate-mcq`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ topic: t, examTrack: track, mockTest }),
      });

      const data = await res.json().catch(() => ({}));

      if (!res.ok) {
        throw new Error(data.error || `Request failed (${res.status})`);
      }

      const list = Array.isArray(data.questions) ? data.questions : [];
      if (list.length < 1) {
        throw new Error("No questions returned — try a different phrasing");
      }

      setSessionTopic(data.topic || t);
      setSessionExamTrack(data.examTrack || track);
      setQuestions(list);
      setCurrentIndex(0);
      setSelectedAnswer(null);
      setShowFeedback(false);
      setCorrectCount(0);
      setAttemptedCount(0);
      setWrongItems([]);
      wrongCountBySubtopicRef.current = {};
      weakAddedBySubtopicRef.current = {};
      setPhase("quiz");
    } catch (err) {
      setError(err.message || "Could not generate questions. Is the server running?");
    } finally {
      setLoading(false);
    }
  };

  const isCorrect =
    question &&
    selectedAnswer !== null &&
    selectedAnswer === question.correctIndex;

  const handleSubmit = () => {
    if (selectedAnswer === null || showFeedback || !question) return;

    const correct = selectedAnswer === question.correctIndex;

    setCorrectCount((c) => c + (correct ? 1 : 0));
    setAttemptedCount((a) => a + 1);

    if (!correct) {
      const sub = question.subtopic || sessionTopic;
      setWrongItems((prev) => [
        ...prev,
        {
          subtopic: sub,
          question: question.question,
          explanation: question.explanation,
          correctOption: question.options[question.correctIndex],
          sourceLabel: question.sourceLabel || "",
        },
      ]);
      if (sub) {
        const subLabel = String(sub).trim();
        const subKey = subLabel.toLowerCase();
        wrongCountBySubtopicRef.current[subKey] =
          (wrongCountBySubtopicRef.current[subKey] || 0) + 1;

        const wrongCount =
          wrongCountBySubtopicRef.current[subKey] || 0;

        if (
          wrongCount >= WEAK_TOPIC_INCORRECT_THRESHOLD &&
          !weakAddedBySubtopicRef.current[subKey]
        ) {
          weakAddedBySubtopicRef.current[subKey] = true;
          dispatch(recordPracticeWeakTopics([subLabel]));
        }
      }
    }

    setShowFeedback(true);
  };

  const handleNext = () => {
    if (!showFeedback) return;

    if (currentIndex >= totalQuestions - 1) {
      setPhase("results");
      setSelectedAnswer(null);
      setShowFeedback(false);
      return;
    }

    setCurrentIndex((i) => i + 1);
    setSelectedAnswer(null);
    setShowFeedback(false);
  };

  const handleBackToInput = () => {
    setPhase("input");
    resetSessionState();
    setTopicInput("");
  };

  const handleAnswerSelect = (index) => {
    if (showFeedback) return;
    setSelectedAnswer(index);
  };

  const wrongSubtopicsUnique = useMemo(() => {
    const seen = new Set();
    const list = [];
    wrongItems.forEach((w) => {
      const key = w.subtopic || "General";
      if (!seen.has(key)) {
        seen.add(key);
        list.push(key);
      }
    });
    return list;
  }, [wrongItems]);

  const mockTestScore100 = useMemo(() => {
    if (!isMockTestSession || attemptedCount < 1) return null;
    const wrong = attemptedCount - correctCount;
    const rawScore = correctCount * 4 - wrong;
    const maxPossibleScore = totalQuestions * 4;
    if (maxPossibleScore <= 0) return 0;
    return Math.max(0, Math.min(100, Math.round((rawScore / maxPossibleScore) * 100)));
  }, [isMockTestSession, attemptedCount, correctCount, totalQuestions]);

  useEffect(() => {
    if (phase !== "results" || !isMockTestSession || mockTestScore100 == null) return;
    if (mockSaveDoneRef.current) return;
    mockSaveDoneRef.current = true;
    dispatch(setMockTestScore({ score: mockTestScore100 }));
    const s = store.getState();
    saveUserProfile({
      plannerWeakTopics: s.knowledge.plannerWeakTopics,
      spacedWeakTopics: s.knowledge.spacedWeakTopics,
      practiceWeakTopics: s.knowledge.practiceWeakTopics,
      concepts: s.knowledge.concepts,
      adaptiveStats: s.study.adaptiveStats,
      flashcards: s.study.flashcards,
      progressStats: s.progress,
    }).catch(() => {});
  }, [phase, isMockTestSession, mockTestScore100, dispatch]);

  const panelStyle = {
    border: "2px solid var(--border-hard)",
    borderRadius: "4px",
    background: "var(--card)",
  };

  /* ---------- INPUT ---------- */
  if (phase === "input") {
    return (
      <div className="p-8 space-y-6 max-w-4xl relative overflow-hidden">
        {/* Stickers */}
        <div className="sticker sticker-slow" style={{ top: "4%", right: "2%", fontSize: "2.5rem", "--sticker-rot": "-12deg" }}>🎯</div>
        <div className="sticker sticker-drift" style={{ top: "35%", right: "3%", fontSize: "2rem", "--sticker-rot": "8deg", animationDelay: "1s" }}>📝</div>

        <div className="mb-2">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-1 h-8 rounded-full" style={{ background: "#C0392B" }} />
            <h1 className="text-3xl font-black text-foreground" style={{ fontFamily: "'Merriweather', Georgia, serif" }}>
              Adaptive Practice
            </h1>
          </div>
          <p className="text-muted-foreground ml-4">
            Pick an exam track, then enter a topic or chapter. You’ll get 10–15
            PYQ-style MCQs (labeled with exam + year, or “PYQ-style” if not a
            verified past paper). Accuracy updates after each answer.
          </p>
          <p className="text-xs text-muted-foreground mt-2 ml-4">
            Lifetime (saved):{" "}
            <span className="font-semibold text-accent4">{lifetime.correctTotal}</span>{" "}
            correct ·{" "}
            <span className="font-semibold text-accent">{lifetime.incorrectTotal}</span>{" "}
            incorrect — weak topics feed your knowledge graph &amp; spaced review.
          </p>
        </div>

        <div className="p-6 space-y-4 shadow-sm" style={panelStyle}>
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">
            <GraduationCap className="w-4 h-4 text-accent2" />
            Exam track
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {EXAM_OPTIONS.map((opt) => (
              <button
                key={opt.id}
                type="button"
                onClick={() => setExamTrack(opt.id)}
                className={`text-left border-2 px-4 py-3 transition-all ${
                  examTrack === opt.id
                    ? "bg-accent/10 border-accent text-foreground"
                    : "border-border bg-card hover:border-accent/40 text-muted-foreground hover:text-foreground"
                }`}
                style={{ borderRadius: "4px" }}
              >
                <p className={`font-bold ${examTrack === opt.id ? "text-accent" : "text-foreground"}`}>{opt.title}</p>
                <p className="text-xs text-muted-foreground mt-1">{opt.blurb}</p>
              </button>
            ))}
          </div>

          <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground">
            Topic / chapter / concept
          </label>
          <textarea
            value={topicInput}
            onChange={(e) => setTopicInput(e.target.value)}
            placeholder="e.g. Thermodynamics, Chemical bonding, Modern Indian History…"
            rows={5}
            className="w-full px-4 py-3 bg-background text-foreground placeholder-muted-foreground/60 focus:outline-none focus:border-accent resize-y min-h-[120px]"
            style={{ border: "2px solid var(--border-hard)", borderRadius: "4px" }}
          />
          {error && (
            <p className="text-sm font-medium text-accent" role="alert">
              ⚠ {error}
            </p>
          )}
          <button
            type="button"
            onClick={handleGenerate}
            disabled={loading || !topicInput.trim()}
            className="inline-flex items-center gap-2 px-6 py-3 bg-accent text-white text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-95 transition-opacity"
            style={{ borderRadius: "4px" }}
          >
            {loading ? (
              <>
                <Loader2 className="w-5 h-5 animate-spin" />
                Generating quiz…
              </>
            ) : (
              <>
                <Sparkles className="w-5 h-5" />
                {/full\s*pyq|full\s*paper|mock|all\s*chapters/i.test(topicInput)
                  ? "Generate 60 questions (Full PYQ)"
                  : "Generate 15–20 questions"}
              </>
            )}
          </button>
          {loading && (
            <div className="mt-4">
              <AILoader />
            </div>
          )}
        </div>
      </div>
    );
  }

  /* ---------- RESULTS ---------- */
  if (phase === "results") {
    const rawScore = correctCount * 4 - (attemptedCount - correctCount);
    const mockScore =
      isMockTestSession && attemptedCount > 0
        ? Math.max(0, Math.min(100, Math.round((rawScore / (totalQuestions * 4)) * 100)))
        : null;
    return (
      <div className="p-8 space-y-6 max-w-4xl relative overflow-hidden">
        {/* Stickers */}
        <div className="sticker sticker-slow" style={{ top: "4%", right: "2%", fontSize: "2.5rem", "--sticker-rot": "-12deg" }}>🎯</div>
        <div className="sticker sticker-drift" style={{ top: "35%", right: "3%", fontSize: "2rem", "--sticker-rot": "8deg", animationDelay: "1s" }}>📝</div>

        <div>
          <h1 className="text-3xl font-black text-foreground mb-2" style={{ fontFamily: "'Merriweather', Georgia, serif" }}>Session complete</h1>
          <p className="text-muted-foreground">
            <span className="text-accent2 font-bold">{sessionExamTrack}</span> ·{" "}
            Topic: <span className="text-foreground font-semibold">{sessionTopic}</span> ·{" "}
            {totalQuestions} questions ·{" "}
            <span className="text-accent4 font-bold">{accuracyPct}%</span> accuracy (
            {correctCount}/{attemptedCount} correct)
          </p>
          {mockScore != null && (
            <div className="mt-4 border-2 bg-accent/5 px-6 py-4" style={{ borderColor: "var(--border-hard)", borderRadius: "4px" }}>
              <p className="text-xs font-bold uppercase tracking-wider text-accent mb-1">Mock test score (100 marks, +4 / −1)</p>
              <p className="text-3xl font-black text-foreground">{mockScore} / 100</p>
              <p className="text-xs text-muted-foreground mt-2">Saved to your profile for Progress.</p>
            </div>
          )}
        </div>

        <div className="p-6 space-y-6 shadow-sm" style={panelStyle}>
          <div className="flex items-center gap-2">
            <BookOpen className="w-6 h-6 text-accent2" />
            <h2 className="text-xl font-black text-foreground" style={{ fontFamily: "'Merriweather', Georgia, serif" }}>
              Revise these areas
            </h2>
          </div>

          {wrongItems.length === 0 ? (
            <div className="border-2 bg-accent4/5 px-4 py-6 text-center" style={{ borderColor: "var(--accent4)", borderRadius: "4px" }}>
              <CheckCircle className="w-10 h-10 text-accent4 mx-auto mb-2" />
              <p className="text-foreground font-bold">Perfect score</p>
              <p className="text-sm text-muted-foreground mt-1">
                No weak spots flagged for this round.
              </p>
            </div>
          ) : (
            <>
              <p className="text-sm text-muted-foreground">
                Subtopics to revisit:{" "}
                <span className="text-accent font-bold">
                  {wrongSubtopicsUnique.join(" · ")}
                </span>
              </p>
              <ul className="space-y-4">
                {wrongItems.map((w, i) => (
                  <li
                    key={i}
                    className="border bg-background p-4"
                    style={{ borderRadius: "4px", borderColor: "var(--border)" }}
                  >
                    <p className="text-xs font-bold uppercase tracking-wide text-accent mb-1">
                      {w.subtopic}
                      {w.sourceLabel ? (
                        <span className="text-muted-foreground font-normal normal-case ml-2">
                          · {w.sourceLabel}
                        </span>
                      ) : null}
                    </p>
                    <p className="text-foreground text-sm font-semibold mb-2">{w.question}</p>
                    <p className="text-xs text-muted-foreground mb-1">
                      Correct answer:{" "}
                      <span className="text-accent4 font-bold">{w.correctOption}</span>
                    </p>
                    <p className="text-xs text-muted-foreground leading-relaxed">{w.explanation}</p>
                  </li>
                ))}
              </ul>
            </>
          )}

          <div className="flex flex-wrap gap-3 pt-2">
            <button
              type="button"
              onClick={handleBackToInput}
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-accent text-white text-sm font-bold hover:opacity-90 transition-opacity"
              style={{ borderRadius: "4px" }}
            >
              <RotateCcw className="w-4 h-4" />
              New topic
            </button>
            <button
              type="button"
              onClick={() => {
                const t = sessionTopic;
                const e = sessionExamTrack;
                resetSessionState();
                setTopicInput(t);
                setExamTrack(e);
                setPhase("input");
              }}
              className="px-5 py-2.5 border-2 text-foreground font-bold hover:bg-muted/30 text-sm"
              style={{ borderColor: "var(--border-hard)", borderRadius: "4px" }}
            >
              Edit topic & regenerate
            </button>
          </div>
        </div>
      </div>
    );
  }

  /* ---------- QUIZ ---------- */
  if (!question) {
    return (
      <div className="p-8 text-muted-foreground relative">
        No question loaded.{" "}
        <button type="button" className="text-accent font-bold underline" onClick={handleBackToInput}>
          Start over
        </button>
      </div>
    );
  }

  return (
    <div className="p-8 space-y-6 relative overflow-hidden max-w-7xl mx-auto">
      {/* Stickers */}
      <div className="sticker sticker-slow" style={{ top: "4%", right: "2%", fontSize: "2.5rem", "--sticker-rot": "-12deg" }}>🎯</div>
      <div className="sticker sticker-drift" style={{ top: "35%", right: "3%", fontSize: "2rem", "--sticker-rot": "8deg", animationDelay: "1s" }}>⏰</div>
      <div className="sticker sticker-wobble" style={{ bottom: "12%", right: "4%", fontSize: "1.8rem", "--sticker-rot": "-6deg", animationDelay: "0.6s" }}>🧠</div>

      <div className="mb-2 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-1 h-8 rounded-full" style={{ background: "#C0392B" }} />
            <h1 className="text-3xl font-black text-foreground" style={{ fontFamily: "'Merriweather', Georgia, serif" }}>
              Adaptive Practice
            </h1>
          </div>
          <p className="text-muted-foreground text-sm ml-4">
            <span className="text-accent2 font-bold">{sessionExamTrack}</span> ·{" "}
            <span className="text-foreground font-semibold">{sessionTopic}</span> ·{" "}
            <span className="text-accent3 font-bold">{totalQuestions}</span>{" "}
            {isMockTestSession ? "mock test questions (+4/−1)" : "PYQ-style questions"}
          </p>
        </div>
      </div>

      {isMockTestSession && attemptedCount > 0 && (
        <div className="border bg-accent2/5 px-4 py-2 text-sm text-accent2 font-bold" style={{ borderColor: "var(--accent2)", borderRadius: "4px" }}>
          Live mock marks:{" "}
          <span className="font-black text-foreground">
            {Math.max(0, Math.min(100, correctCount * 4 - (attemptedCount - correctCount)))}
          </span>
          /100
        </div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
        <div className="p-5 shadow-sm" style={panelStyle}>
          <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">Accuracy (live)</p>
          <p className="text-2xl font-black text-accent4">{accuracyPct}%</p>
          <p className="text-xs text-muted-foreground mt-1">
            {correctCount} correct / {attemptedCount} attempted
          </p>
        </div>
        <div className="p-5 shadow-sm" style={panelStyle}>
          <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">Progress</p>
          <p className="text-2xl font-black text-accent3">
            {currentIndex + 1} / {totalQuestions}
          </p>
          <p className="text-xs text-muted-foreground mt-1">Current question</p>
        </div>
        <div className="p-5 shadow-sm" style={panelStyle}>
          <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">Misses so far</p>
          <p className="text-2xl font-black text-accent">{incorrectCount}</p>
          <p className="text-xs text-muted-foreground mt-1">Updates each submit</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="p-8 shadow-lg space-y-6" style={panelStyle}>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-2">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 bg-accent2/10 text-accent2 border border-accent2/35 rounded-sm">
                    {sessionExamTrack} · PYQ
                  </span>
                  <span className="text-sm font-semibold text-muted-foreground">
                    {question.subtopic || sessionTopic}
                  </span>
                </div>
                {question.sourceLabel ? (
                  <p className="text-sm text-accent font-bold">
                    {question.sourceLabel}
                  </p>
                ) : null}
              </div>
              <div className="flex items-center gap-2 text-muted-foreground text-xs font-bold uppercase tracking-wider shrink-0">
                <Clock className="w-4 h-4 text-accent" />
                Session
              </div>
            </div>

            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">
                Question {currentIndex + 1} of {totalQuestions}
              </p>
              <h2 className="text-xl text-foreground font-bold leading-relaxed">
                {question.question}
              </h2>
            </div>

            <div className="space-y-3">
              {question.options.map((option, index) => {
                const isSelected = selectedAnswer === index;
                const isCorrectAnswer = index === question.correctIndex;

                let optionStyle = {
                  background: "var(--card)",
                  borderColor: "var(--border)",
                  borderWidth: "2px",
                  borderRadius: "4px",
                };

                if (showFeedback) {
                  if (isCorrectAnswer) {
                    optionStyle.background = "rgba(30, 132, 73, 0.1)";
                    optionStyle.borderColor = "var(--accent4)";
                  } else if (isSelected && !isCorrect) {
                    optionStyle.background = "rgba(192, 57, 43, 0.1)";
                    optionStyle.borderColor = "var(--accent)";
                  }
                } else if (isSelected) {
                  optionStyle.background = "rgba(27, 37, 64, 0.08)";
                  optionStyle.borderColor = "var(--border-hard)";
                }

                return (
                  <button
                    key={index}
                    type="button"
                    onClick={() => handleAnswerSelect(index)}
                    disabled={showFeedback}
                    className="w-full text-left p-4 transition-all duration-150 border-2"
                    style={optionStyle}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-foreground">
                        <span className="text-muted-foreground font-mono font-bold mr-2">
                          {String.fromCharCode(65 + index)}.
                        </span>
                        {option}
                      </span>
                      {showFeedback && isCorrectAnswer && (
                        <CheckCircle className="w-5 h-5 shrink-0 text-accent4" />
                      )}
                      {showFeedback && isSelected && !isCorrect && (
                        <XCircle className="w-5 h-5 shrink-0 text-accent" />
                      )}
                    </div>
                  </button>
                );
              })}
            </div>

            {showFeedback && (
              <div
                className="p-4 border-2"
                style={{
                  background: isCorrect ? "rgba(30, 132, 73, 0.05)" : "rgba(192, 57, 43, 0.05)",
                  borderColor: isCorrect ? "var(--accent4)" : "var(--accent)",
                  borderRadius: "4px",
                }}
              >
                <div className="flex items-center gap-2 mb-2">
                  {isCorrect ? (
                    <>
                      <CheckCircle className="w-5 h-5 text-accent4" />
                      <span className="font-bold text-accent4 text-sm uppercase tracking-wider">Correct</span>
                    </>
                  ) : (
                    <>
                      <XCircle className="w-5 h-5 text-accent" />
                      <span className="font-bold text-accent text-sm uppercase tracking-wider">Incorrect</span>
                    </>
                  )}
                </div>
                <p className="text-sm text-foreground leading-relaxed">{question.explanation}</p>
              </div>
            )}

            <div className="flex gap-3">
              {!showFeedback ? (
                <button
                  type="button"
                  onClick={handleSubmit}
                  disabled={selectedAnswer === null}
                  className={`
                    flex-1 py-3 text-sm font-bold transition-all
                    ${selectedAnswer !== null
                      ? "bg-accent text-white hover:opacity-90"
                      : "bg-muted/30 text-muted-foreground cursor-not-allowed border-2 border-border"
                    }
                  `}
                  style={{ borderRadius: "4px" }}
                >
                  Submit answer
                </button>
              ) : (
                <button
                  type="button"
                  onClick={handleNext}
                  className="flex-1 py-3 bg-accent text-white rounded hover:opacity-90 transition-opacity text-sm font-bold"
                  style={{ borderRadius: "4px" }}
                >
                  {currentIndex >= totalQuestions - 1
                    ? "View summary"
                    : "Next question"}
                </button>
              )}
            </div>
          </div>

          <div className="p-4 shadow-sm space-y-2" style={panelStyle}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Answered in session</span>
              <span className="text-xs font-bold text-foreground">
                {attemptedCount} / {totalQuestions}
              </span>
            </div>
            <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
              <div
                className="h-full bg-accent transition-all duration-300"
                style={{
                  width: `${totalQuestions ? (attemptedCount / totalQuestions) * 100 : 0}%`,
                }}
              />
            </div>
          </div>
        </div>

        <div className="space-y-4">
          <div className="p-6 shadow-sm space-y-4" style={panelStyle}>
            <div className="flex items-center gap-2">
              <Target className="w-5 h-5 text-accent" />
              <h2 className="text-lg font-black text-foreground" style={{ fontFamily: "'Merriweather', Georgia, serif" }}>This session</h2>
            </div>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Correct</span>
                <span className="text-accent4 font-bold">{correctCount}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Incorrect</span>
                <span className="text-accent font-bold">{incorrectCount}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Accuracy</span>
                <span className="text-accent3 font-bold">{accuracyPct}%</span>
              </div>
              <div className="flex justify-between border-t pt-3" style={{ borderColor: "var(--border)" }}>
                <span className="text-muted-foreground">Total questions</span>
                <span className="text-foreground font-semibold">{totalQuestions}</span>
              </div>
            </div>
          </div>

          <div className="p-6 shadow-sm space-y-4" style={panelStyle}>
            <div className="flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-accent2" />
              <h2 className="text-lg font-black text-foreground" style={{ fontFamily: "'Merriweather', Georgia, serif" }}>Wrong so far</h2>
            </div>
            {wrongItems.length === 0 ? (
              <p className="text-xs text-muted-foreground">No misses yet — keep going.</p>
            ) : (
              <ul className="space-y-2">
                {wrongSubtopicsUnique.map((s) => (
                  <li
                    key={s}
                    className="text-xs font-bold text-accent border px-3 py-2 bg-accent/5"
                    style={{ borderColor: "var(--accent)30", borderRadius: "2px" }}
                  >
                    {s}
                  </li>
                ))}
              </ul>
            )}
          </div>

          <button
            type="button"
            onClick={handleBackToInput}
            className="w-full py-2.5 border-2 text-muted-foreground hover:text-foreground hover:bg-muted/30 text-xs font-bold transition-all uppercase tracking-wider"
            style={{ borderColor: "var(--border-hard)", borderRadius: "4px" }}
          >
            Cancel & new topic
          </button>
        </div>
      </div>
    </div>
  );
}
